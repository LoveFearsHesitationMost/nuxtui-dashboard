...
class UserDashboard < Administrate::BaseDashboard
  ...
  ATTRIBUTE_TYPES = {
    id: Field::Number,
    admin: Field::Boolean.with_options(searchable: true),
    email: Field::String.with_options(searchable: true),
    password: Field::Password,
    password_confirmation: Field::Password,
    # password_digest: Field::String.with_options(destroyable: false),
  ...
  def display_resource(user)
    "#{user.email} (#{user.admin? ? '管理员' : '普通用户'})"
  end

  # Override this method to customize how sessions are counted
  # def sessions_count

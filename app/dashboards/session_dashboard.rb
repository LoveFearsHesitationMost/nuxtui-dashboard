...
class SessionDashboard < Administrate::BaseDashboard
  ...
  ATTRIBUTE_TYPES = {
    ...
    user: Field::BelongsTo.with_options(
      searchable: true,
      searchable_fields: %w[email id],
      ),
    user_agent: Field::String.with_options(searchable: true),
    ip_address: Field::String.with_options(searchable: true),
  ...
  #   }.freeze
  COLLECTION_FILTERS = {
    admin: ->(resources) { resources.joins(:user).where users: { admin: true } }
  }.freeze
